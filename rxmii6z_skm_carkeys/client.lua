    ...
    while true do
        ...
        if(IsControlJustPressed(1, 303)) then 
            ...
            if(IsPedInAnyVehicle(plr, true))then
                ...
                if (GetVehicleNumberPlateText(localVehId)) == nil then
                    --(Patch : Le script crashais sans ça)
                    TriggerEvent('esx:showNotification', 'Impossible de fermer le véhicule pour le moment')
             else
                ...
                if (GetVehicleNumberPlateText(localVehId)) == nil then
                    --Si aucun véhicule à proximité (Patch : Le script crashais sans ça)
                    TriggerEvent('esx:showNotification', 'Aucun véhicule à proximité')
                else
                    local localVehPlate = string.lower(GetVehicleNumberPlateText(localVehId))
